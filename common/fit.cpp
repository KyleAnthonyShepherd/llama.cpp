...
common_device_memory_data_vec common_get_device_memory_data(
        ...
        uint32_t & hp_n_expert,
        ggml_log_level log_level) {
    uint32_t hp_neu = 0; // hparams.n_expert_used, not exposed by this wrapper
    std::vector<llama_device_memory_data> impl = common_get_device_memory_data_impl(
            path_model, mparams, cparams, devs, hp_ngl, hp_n_ctx_train, hp_n_expert, hp_neu, log_level);
...
static void common_params_fit_impl(
        ...
        if (dmds_extra.empty() || n_ctx_extra != cparams->n_ctx) {
            ...
            try {
                measured = common_get_device_memory_data_impl(
                    extra->path_model, extra->mparams, extra->cparams, devs_extra, ngl_extra, nct_extra, nex_extra, neu_extra, log_level);
    ...
    dmds_t dmds_full = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_neu, log_level);
    ...
    if (n_ctx_auto) {
        ...
        if (n_seq_max > 1) {
            ...
            dmds_full = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_neu, log_level);
        }
    }
        ...
        if (global_surplus < 0) {
            ...
            if (n_ctx_auto) {
                if (n_ctx_max > n_ctx_min_total) {
                    ...
                    dmds_t dmds_min_ctx = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_neu, log_level);
    ...
    auto get_memory_for_layers = [&](
        ...
        dmds_t dmd_nl = common_get_device_memory_data_impl(
            path_model, &mparams_copy, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_neu, log_level);
    ...
    if (hp_nex > 0) {
        ...
        dmds_t dmds_cpu_moe = common_get_device_memory_data_impl(
            path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_neu, log_level);
...
void common_fit_print(
    ...
    auto dmd = common_get_device_memory_data_impl(path_model, mparams, cparams, devs, hp_ngl, hp_nct, hp_nex, hp_neu, GGML_LOG_LEVEL_ERROR);
