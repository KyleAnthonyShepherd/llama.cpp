...
struct common_params {
    ...
    int32_t kv_spill_margin    = 256;   // MiB of VRAM a growing KV cache keeps free before it moves layers to host memory
    ...
    int  op_offload_min_batch = 0;  // batch at which a host-weight op offloads to the device
                                    // 0 = leave the backend default (32); -ehs raises it to 128
    ...
    bool mmproj_weights_host = false;           // keep mmproj weights in RAM, run the encode on the GPU
    bool mmproj_compute_lazy = false;           // allocate the mmproj compute buffers only while encoding
