...
private:
    ...
    std::unique_ptr<server_res_generator> handle_slots_drop(const server_http_req & req, int id_slot);
    std::unique_ptr<server_res_generator> parse_slot_store(const json & request_data, const std::string & filename, struct server_task::slot_action & out);
