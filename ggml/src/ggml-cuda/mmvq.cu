...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    const auto should_use_small_k = [&](int c_ncols_dst) {
        ...
        // PTQ1_0 is bound by the trit decode, not by bytes: one row per lane leaves too few loads in
        // flight, nwarps rows per lane is ~1.5x faster at K = 17408 and never slower at K = 5120
        if (type == GGML_TYPE_PTQ1_0 && c_ncols_dst == 1 && GGML_CUDA_CC_IS_NVIDIA(cc)) {
            use = true;
        }
